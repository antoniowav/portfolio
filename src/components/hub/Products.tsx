...
function ProductRow({
  ...
  const rowClass =
    'group grid grid-cols-12 items-center gap-x-4 border-t border-line-strong py-7 transition-colors duration-300 hover:bg-paper-sink sm:py-9'
  ...
  const inner = (
    <>
      ...
      <div className="col-span-10 sm:col-span-7">
        <h3 className="text-[clamp(2rem,6vw,4.5rem)] font-bold leading-[0.95] tracking-[-0.03em] transition-transform duration-300 ease-expo group-hover:translate-x-2">
          {p.name}
        </h3>
  ...
  return (
    <Panel id="products" aria-labelledby="products-heading">
      <div className="mx-auto w-full max-w-shell px-6 lg:px-10 lg:pr-24">
        ...
        <h2 className="reveal-item d1 mt-10 max-w-[16ch] text-[clamp(1.75rem,4vw,3rem)] font-bold leading-[1] tracking-[-0.03em]">
          What I am building, one at a time.
        </h2>

        <ul className="mt-10 border-b border-line-strong lg:mt-14">
          {products.map((p, n) => (
            <ProductRow
              key={p.name}
              p={p}
